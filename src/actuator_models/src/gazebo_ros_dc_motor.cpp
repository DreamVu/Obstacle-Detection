...
#include "nowtech_actuator_models/gazebo_ros_dc_motor.h"
...
void GazeboRosMotor::Load ( physics::ModelPtr _parent, sdf::ElementPtr _sdf ) {
    ...
    // global parameters
    gazebo_ros_->getParameter<std::string> ( command_topic_,  "command_topic",  "/motor/voltage_norm" ); // should be just "input"
    ...
    ros::SubscribeOptions soc = ros::SubscribeOptions::create<std_msgs::Float32> (
        command_topic_,
        1,
    ...
    cmd_vel_subscriber_ = gazebo_ros_->node()->subscribe(soc);
    ROS_INFO_NAMED(plugin_name_, "%s: Subscribed to %s", gazebo_ros_->info(), command_topic_.c_str());

    // brake command subscriber
    ros::SubscribeOptions sob = ros::SubscribeOptions::create<std_msgs::Bool> (
        "brake",
        1,
        boost::bind(&GazeboRosMotor::brakeCommandCallBack, this, _1),
        ros::VoidPtr(),
        &queue_
    );
    brake_cmd_subscriber_ = gazebo_ros_->node()->subscribe(sob);
    brake_locked_ = false;
    ROS_INFO_NAMED(plugin_name_, "%s: Subscribed to %s", gazebo_ros_->info(), "/brake");
    ...
    dynamic_reconfigure_server_.reset(new dynamic_reconfigure::Server<nowtech_actuator_models::motorModelConfig>(reconf_mutex_, ros::NodeHandle(*node_handle_)));
...
void GazeboRosMotor::reconfigureCallBack(const nowtech_actuator_models::motorModelConfig &config, uint32_t level) {
...
void GazeboRosMotor::motorModelUpdate(double dt, double output_shaft_omega, double actual_load_torque) {
    ...
    double V = input_ * supply_voltage_; // input voltage (command input for motor velocity)
...
void GazeboRosMotor::brakeCommandCallBack ( const std_msgs::Bool::ConstPtr& cmd_msg ) {
